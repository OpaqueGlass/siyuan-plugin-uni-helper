...
import { warnPush, errorPush, debugPush, logPush } from "../core/logger"
/**向思源api发送请求
...
export function getNotebookSortModeF(notebookId = undefined) {
    try {
        ...
        let notebookSortMode = window.document.querySelector(`.file-tree.sy__file ul[data-url='${notebookId}']`)?.getAttribute("data-sortmode") ?? (getNotebookInfoLocallyF(notebookId) as INotebook)?.sortMode;
