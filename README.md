...
思源笔记（SiYuan）插件通用工具包。把各插件仓库里反复复制的 **设置面板框架 + 思源 API 封装 + 常用 util + 日志/语言层** 收敛为一个 npm 包，插件只需升级依赖版本即可同步基础能力的修复与增强。

包含四块能力：

| 模块 | 说明 |
| --- | --- |
| **设置面板框架**（`siyuan-plugin-uni-helper/settings`） | 声明式注册设置项（沿用各插件现有的 setting property 形式），内置搜索，支持设置项级 / 标签页级 / 控件级三级自定义 Vue 组件 |
| **思源 API**（`siyuan-plugin-uni-helper/api`） | `syapi` 全集原样搬迁，函数形态与返回结构不变，**不套任何包装器或版本标注** |
| **核心层**（`siyuan-plugin-uni-helper/core`） | `registerPlugin` / 分级 logger / i18n 三级回退 / `@SyVersion` 版本守卫 / 互斥锁 / 版本比较等 |
| **设置面板 UI**（`siyuan-plugin-uni-helper/vue`） | 面板与各设置项控件（SFC），一般无需直接引用 |

设计取向：**包不修改、不覆盖插件实例**。设置面板的实际打开时机仍由插件自己的 `openSetting()` 决定，包只提供实现供其显式调用；存储文件名与文件格式保持不变，老版本插件升级后用户设置不丢失。
...
在 `onload()` 中最早调用，它一次性完成「保存插件实例 + 初始化日志器 + 装载语言包」三件事：
...
包内所有能力通过 `PluginContext` 反查插件，**不会反向 import 插件代码**；未注册就使用需要实例的能力时，`usePluginContext()` 会直接抛错以便定位。

### 2. 设置引擎

插件侧只保留「业务配置 + 迁移 + 校验钩子」，加载、落盘、类型校正、防抖、调试开关由包统一处理：
...
3. `checkSettingType()`：按 `ConfigProperty` 声明校正类型（`SELECT` 越界回退默认值、`SWITCH` 缺失回退、 `NUMBER` 钳制 `min`/`max` 且 `zeroToMaxKeys` 中的 0 回退到 `max`），最后执行插件的 `customValidate()`
...
内置类型：`SELECT` / `TEXT` / `NUMBER` / `BUTTON` / `TEXTAREA` / `SWITCH` / `ORDER` / `PATH` / `TIPS` / `CUSTOM`，也允许扩展自定义类型名（如 `CUSTOM_NOTEBOOK`，需配合 `component`）。

`ConfigProperty` 的常用字段：

| 字段 | 说明 |
| --- | --- |
| `key` | 设置项 key，同时决定 i18n 词条名（见「i18n 约定」） |
| `type` | 类型 |
| `min` / `max` | `NUMBER` 的区间 |
...
内部完成：UUID → `createApp(SettingPanel)` → `siyuan.Dialog` → 挂载 → `destroyCallback` 中 `app.unmount()`。默认尺寸移动端 `92vw / 50vw`、桌面端 `1040px / 80vh`。
...
- 未搬迁宿主侧 `src/utils/common.ts`；其中被依赖的 `getToken()` 独立为 `api/token.ts`，可通过 `setTokenProvider(fn)` 注入真实实现，未注入时仍返回 `""`（保持历史行为）
...
包内的语言层只做三件事：**插件词条优先、框架词条兜底、key 本身最后兜底**。词条缺失不会抛错，只会显示 key 本身，因此新增设置项时若忘记补词条，界面上会直接暴露 key 名。
...
`settingLang(key)` 返回 `[name, desp, btnName]` 三元组（对应 `_name` / `_desp` / `_btn`），`settingPageLang(key)` 返回 `[pageName]`（对应 `settingpage_{key}_name`）。
...
本包 devDependencies 与各插件仓库升级到 vite `^6.4.3` 时，配套依赖同步版本如下（已核对 peer / engines，Node 需 `^20.19.0 || >=22.12.0`）：
...
- vite 6 下不要再把整个 `process.env` 注入 `define`（会告警且泄露环境变量）；只显式注入用到的字段，**必须包含 `process.env.NODE_ENV`**，否则 Vue 会保留运行时判断并把 dev 分支打进产物（体积变大且出现 `[Vue warn]`）
...
- [SiYuan 思源笔记](https://github.com/siyuan-note/siyuan)：本包的所有 API 封装与设置面板样式均基于其插件体系
- [siyuan 插件 API 类型包](https://www.npmjs.com/package/siyuan) 与官方插件开发文档
- 设置项声明式注册、设置面板 UI、`syapi` 封装等设计沿用 OpaqueGlass 系列插件在这些仓库中沉淀的既有约定，在此感谢原作者的开源工作
- 参与试点的插件仓库（层级导航等）提供的真实迁移反馈
